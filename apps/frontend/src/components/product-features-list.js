...
exports.default = ProductFeaturesList;
 ...
 */
function ProductFeaturesList({ description, locale }) {
    ...
    return (<FeaturesListWithTooltips features={features}/>);
}
/**
 * Component that renders features list with tooltip support
 */
function FeaturesListWithTooltips({ features }) {
    ...
    return (<div ref={containerRef} className="space-y-2">
      {features.map((feature, index) => {
            ...
            return (<div key={index} className="flex items-center gap-3 text-base py-1">
            <span className="font-bold text-brand-primary min-w-[3.5rem] text-right text-lg shrink-0">
              {displayValue}
            </span>
            <span className="text-foreground flex-1 leading-relaxed" dangerouslySetInnerHTML={{ __html: feature.nameHtml || feature.name }}/>
          </div>);
        })}
    </div>);
}
//# sourceMappingURL=product-features-list.js.map