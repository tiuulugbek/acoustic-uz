...
exports.default = ServiceContent;
 ...
 * - Paragraphs
 */
function ServiceContent({ content, locale }) {
    const renderedContent = (0, react_1.useMemo)(() => {
        ...
        if (isHTML) {
            ...
            processedContent = (0, content_processor_1.processContentShortcodes)(processedContent);
            ...
            return <content_processor_1.ProcessedContent html={processedContent}/>;
        }
        ...
        const flushList = () => {
            if (currentList.length > 0 && listType) {
                const items = currentList.map((item, idx) => (<li key={`${listKey}-${idx}`} className="mb-2 pl-2">
            {renderInlineContent(item.trim())}
          </li>));
                elements.push(listType === 'ordered' ? (<ol key={`list-${listKey}`} className="mb-4 ml-6 list-decimal space-y-2">
              {items}
            </ol>) : (<ul key={`list-${listKey}`} className="mb-4 ml-6 list-disc space-y-2">
              {items}
            </ul>));
                currentList = [];
                listType = null;
        ...
        const renderInlineContent = (text) => {
            ...
            while ((match = boldRegex.exec(text)) !== null) {
                ...
                parts.push(<strong key={`bold-${key++}`} className="font-semibold text-foreground">
            {match[1]}
          </strong>);
        ...
        lines.forEach((line, index) => {
            ...
            if (trimmed.startsWith('### ')) {
                ...
                elements.push(<h3 key={`h3-${index}`} id={headingId} className="mb-3 mt-6 scroll-mt-20 text-xl font-semibold text-foreground">
            ...
            if (trimmed.startsWith('## ')) {
                ...
                const headingId = `section-${index}`;
                elements.push(<h2 key={`h2-${index}`} id={headingId} className="mb-4 mt-8 scroll-mt-20 text-2xl font-bold text-foreground">
            {renderInlineContent(headingText)}
          </h2>);
                return;
            }
            ...
            // Regular paragraph
            flushList();
            elements.push(<p key={`p-${index}`} className="mb-4 leading-relaxed text-foreground/90">
    ...
    if (!renderedContent || (Array.isArray(renderedContent) && renderedContent.length === 0)) {
        return (<p className="text-muted-foreground" suppressHydrationWarning>
        {locale === 'ru'
