...
exports.processContentShortcodes = processContentShortcodes;
exports.ProcessedContent = ProcessedContent;
 ...
 */
function processContentShortcodes(content) {
    ...
    processed = processed.replace(tableRegex, (match, position, tableContent) => {
        ...
        const className = positionClasses[position] || 'mx-auto';
 ...
 */
function ProcessedContent({ html }) {
    const containerRef = (0, react_1.useRef)(null);
    // Use the optimized tooltip manager hook
    (0, tooltip_manager_1.useTooltipManager)(containerRef);
    return (<div ref={containerRef} className="rich-content prose prose-lg max-w-none" dangerouslySetInnerHTML={{ __html: html }}/>);
}
