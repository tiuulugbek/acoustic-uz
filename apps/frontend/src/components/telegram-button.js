...
exports.default = TelegramButton;
const react_1 = require("react");
const api_1 = require("@/lib/api");
const locale_client_1 = require("@/lib/locale-client");
const lucide_react_1 = require("lucide-react");
function TelegramButton() {
    const [botUsername, setBotUsername] = (0, react_1.useState)(null);
    const [message, setMessage] = (0, react_1.useState)('');
    const [phoneNumber, setPhoneNumber] = (0, react_1.useState)('1385');
    const [isLoading, setIsLoading] = (0, react_1.useState)(true);
    const [isChatBubbleVisible, setIsChatBubbleVisible] = (0, react_1.useState)(true);
    ...
    (0, react_1.useEffect)(() => {
        const fetchSettings = async () => {
            try {
                ...
                const settings = await (0, api_1.getSettings)();
    ...
    return (<>
      ...
      <div className="hidden md:block fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        {/* Chat Bubble */}
        {isChatBubbleVisible && (<div className="relative max-w-xs rounded-2xl bg-white px-4 py-3 shadow-xl animate-in slide-in-from-bottom-4 fade-in">
          {/* Close button */}
          <button onClick={handleClose} className="absolute -top-2 -right-2 flex h-6 w-6 items-center justify-center rounded-full bg-gray-200 text-gray-600 hover:bg-gray-300 transition-colors" aria-label="Yopish">
        ...
        {/* Telegram Button */}
        <a href={`https://t.me/${botUsername}`} onClick={(e) => {
            e.preventDefault();
            handleClick();
        }} className="flex h-14 w-14 items-center justify-center rounded-full bg-white shadow-lg transition-all hover:scale-110 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-[#0088cc] focus:ring-offset-2 animate-in slide-in-from-bottom-4 fade-in" data-social="telegram" target="_blank" rel="noopener noreferrer" aria-label="Telegram orqali yozish" title="Telegram orqali yozish">
          <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48" fill="none" className="h-12 w-12">
            ...
            <path fill="#0088cc" d="m18.693 29.614 12.007 9.01c1.37.768 2.36.37 2.7-1.292l4.888-23.392c.5-2.038-.765-2.962-2.075-2.357l-28.7 11.239c-1.96.798-1.948 1.908-.357 2.403l7.365 2.334 17.05-10.925c.805-.496 1.544-.23.938.317"/>
          </svg>
        </a>
      </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 flex animate-in slide-in-from-bottom-4 fade-in">
        ...
        <a href={`tel:${phoneNumber.replace(/\s/g, '')}`} className="flex-1 flex items-center justify-center gap-2 bg-brand-primary/90 backdrop-blur-sm px-4 py-3 text-white transition-opacity hover:opacity-90" aria-label={locale === 'ru' ? 'Позвонить' : 'Qo\'ng\'iroq qilish'}>
          <lucide_react_1.Phone size={18} className="text-white"/>
        ...
        <a href={`https://t.me/${botUsername}`} onClick={(e) => {
            e.preventDefault();
            handleClick();
        }} className="flex-1 flex items-center justify-center gap-2 bg-[#0088cc]/90 backdrop-blur-sm px-4 py-3 text-white transition-opacity hover:opacity-90" aria-label={locale === 'ru' ? 'Записаться на прием' : 'Qabulga yozilish'}>
