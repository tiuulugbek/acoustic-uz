...
exports.default = FrequencyTest;
const react_1 = require("react");
const lucide_react_1 = require("lucide-react");
function FrequencyTest({ locale, ear, frequencies, onComplete, onBack, playTone, stopTone, updateVolume, isPlaying, volume, isSubmitting = false, }) {
    const isRu = locale === 'ru';
    const [currentIndex, setCurrentIndex] = (0, react_1.useState)(0);
    const [results, setResults] = (0, react_1.useState)({}); // Store volume levels instead of boolean
    const [currentVolume, setCurrentVolume] = (0, react_1.useState)(0.0); // Start at 0% volume
    const autoContinueTimerRef = (0, react_1.useRef)(null);
    ...
    return (<div className="space-y-10 max-w-3xl mx-auto">
      ...
      <div className="max-w-2xl mx-auto px-4 space-y-6">
        ...
        <div className="flex items-center gap-3 md:gap-6">
          ...
          <button onClick={() => handleVolumeChange(currentVolume - 0.05)} disabled={isSubmitting || currentVolume <= 0} className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gray-800 text-white flex items-center justify-center hover:bg-gray-900 active:scale-95 transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-lg touch-manipulation">
            <lucide_react_1.Minus className="w-5 h-5 md:w-6 md:h-6"/>
          </button>

          {/* Slider - Mobile optimized (ReSound style) */}
          <div className="flex-1 relative py-6 md:py-8">
            <input type="range" min="0" max="1" step="0.005" value={currentVolume} onChange={(e) => handleVolumeChange(parseFloat(e.target.value))} onInput={(e) => handleVolumeChange(parseFloat(e.target.value))} className="hearing-test-slider w-full h-3 md:h-4 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-brand-primary touch-manipulation" style={{
          ...
          <button onClick={() => handleVolumeChange(currentVolume + 0.05)} disabled={isSubmitting || currentVolume >= 1} className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gray-800 text-white flex items-center justify-center hover:bg-gray-900 active:scale-95 transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-lg touch-manipulation">
            <lucide_react_1.Plus className="w-5 h-5 md:w-6 md:h-6"/>
          </button>
        </div>
      </div>
      ...
      <div className="flex justify-center pt-4 px-4">
        <button onClick={onBack} disabled={isSubmitting} className="px-6 md:px-8 py-2.5 md:py-3 border-2 border-gray-300 rounded-xl text-gray-700 font-semibold hover:bg-gray-50 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed touch-manipulation text-sm md:text-base">
          {isRu ? '← Назад' : '← Orqaga'}
        </button>
      </div>
