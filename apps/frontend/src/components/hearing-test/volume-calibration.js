...
exports.default = VolumeCalibration;
const react_1 = require("react");
const lucide_react_1 = require("lucide-react");
function VolumeCalibration({ locale, volume, onVolumeChange, onContinue, onBack, playTone, stopTone, isPlaying, }) {
    ...
    return (<div className="space-y-10 max-w-3xl mx-auto">
      ...
      <div className="space-y-8 max-w-2xl mx-auto">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 flex items-center justify-between pt-1">
            ...
            <lucide_react_1.VolumeX className="w-8 h-8 text-gray-400 flex-shrink-0 ml-4"/>
          </div>
        </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 flex items-center justify-between pt-1">
            ...
            <lucide_react_1.Volume2 className="w-8 h-8 text-gray-400 flex-shrink-0 ml-4"/>
          </div>
        </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 flex items-center justify-between pt-1">
            ...
            <lucide_react_1.Headphones className="w-8 h-8 text-gray-400 flex-shrink-0 ml-4"/>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-4 py-8">
        <button onClick={isPlaying ? stopTone : handlePlayTest} className={`w-16 h-16 rounded-full flex items-center justify-center text-white transition-all transform ${isPlaying
            ? 'bg-red-500 hover:bg-red-600 scale-105'
            : 'bg-gray-800 hover:bg-gray-900 hover:scale-105'} shadow-lg`}>
          {isPlaying ? (<lucide_react_1.Square className="w-8 h-8"/>) : (<lucide_react_1.Play className="w-8 h-8 ml-1"/>)}
        </button>
      ...
      {hasPlayed && (<div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-6 max-w-2xl mx-auto">
      ...
      <div className="flex justify-center gap-4 pt-6">
        <button onClick={onBack} className="px-8 py-3 border-2 border-gray-300 rounded-xl text-gray-700 font-semibold hover:bg-gray-50 transition-colors">
          {isRu ? '← Назад' : '← Orqaga'}
        </button>
        <button onClick={() => onContinue(1.0)} // Maksimal volume
     disabled={!hasPlayed} className="px-10 py-3 bg-brand-primary text-white rounded-xl font-bold text-lg hover:bg-brand-primary/90 transition-all transform hover:scale-105 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed disabled:scale-100">
          {isRu ? 'Продолжить →' : 'Davom etish →'}
        </button>
      </div>
    </div>);
}
//# sourceMappingURL=volume-calibration.js.map