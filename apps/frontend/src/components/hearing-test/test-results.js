...
function TestResults({ locale, result, onContact, onRestart }) {
    ...
    const getScoreIcon = (score) => {
        if (!score)
            return <lucide_react_1.Minus className="w-5 h-5"/>;
        if (score >= 80)
            return <lucide_react_1.TrendingUp className="w-5 h-5 text-green-600"/>;
        if (score >= 50)
            return <lucide_react_1.Minus className="w-5 h-5 text-yellow-600"/>;
        return <lucide_react_1.TrendingDown className="w-5 h-5 text-red-600"/>;
    };
    return (<div className="space-y-10 max-w-3xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-4 pt-4">
        <link_1.default href="/branches" className="px-10 py-4 bg-brand-primary text-white rounded-xl font-bold text-lg hover:bg-brand-primary/90 transition-all transform hover:scale-105 shadow-lg text-center">
          {isRu ? 'Найти место для проверки →' : 'Haqiqiy tekshiruv joyini topish →'}
        </link_1.default>
        <button onClick={onContact} className="px-10 py-4 border-2 border-brand-primary text-brand-primary rounded-xl font-bold text-lg hover:bg-brand-primary/10 transition-all transform hover:scale-105">
          {isRu ? 'Связаться с нами' : 'Biz bilan bog\'lanish'}
        </button>
        <button onClick={onRestart} className="px-10 py-4 border-2 border-gray-300 text-gray-700 rounded-xl font-bold text-lg hover:bg-gray-50 transition-all transform hover:scale-105">
