...
exports.default = CatalogFilters;
const link_1 = __importDefault(require("next/link"));
const navigation_1 = require("next/navigation");
function FilterSection({ title, children }) {
    return (<div className="space-y-3 border-b border-border/40 pb-4">
      <h3 className="text-sm font-semibold text-brand-accent">{title}</h3>
      <div className="space-y-2">{children}</div>
    </div>);
}
function FilterCheckbox({ id: _id, label, checked, url, count }) {
    return (<link_1.default href={url} className="flex items-center justify-between gap-2 cursor-pointer text-sm text-muted-foreground hover:text-brand-accent">
...
function CatalogFilters({ categorySlug, locale, brands, selectedBrands, selectedBrandName, selectedAudience, selectedForms, selectedPower, selectedLoss, audienceCounts = {}, formCounts = {}, powerCounts = {}, lossCounts = {}, }) {
    ...
    const hasActiveFilters = selectedBrands.length > 0 || selectedAudience.length > 0 || selectedForms.length > 0 || selectedPower.length > 0 || selectedLoss.length > 0;
    return (<div className="sticky top-24 space-y-6 rounded-2xl border border-border/60 bg-white p-6 shadow-sm">
      <h2 className="text-lg font-semibold text-brand-accent">{locale === 'ru' ? 'Фильтры' : 'Filtrlar'}</h2>

      {/* Brand Filter */}
      {selectedBrandName ? (<FilterSection title={locale === 'ru' ? 'Бренд' : 'Brend'}>
          <div className="flex items-center gap-2 text-sm font-medium text-brand-accent">
            <span>{selectedBrandName}</span>
          </div>
        </FilterSection>) : brands.length > 0 ? (<FilterSection title={locale === 'ru' ? 'Бренд' : 'Brend'}>
          {brands
      ...
      {/* Audience Filter */}
      {Object.values(audienceCounts).some((count) => count > 0) && (<FilterSection title={locale === 'ru' ? 'Для кого' : 'Kimlar uchun'}>
          {audienceOptions
                ...
                return (<FilterCheckbox key={option.value} id={`audience-${option.value}`} label={locale === 'ru' ? option.label_ru : option.label_uz} checked={isSelected} url={url} count={audienceCounts[option.value]}/>);
            })}
        </FilterSection>)}

      {/* Form Factor Filter */}
      {Object.values(formCounts).some((count) => count > 0) && (<FilterSection title={locale === 'ru' ? 'Тип корпуса' : 'Korpus turi'}>
          {formFactorOptions
                ...
                return (<FilterCheckbox key={option.value} id={`form-${option.value}`} label={locale === 'ru' ? option.label_ru : option.label_uz} checked={isSelected} url={url} count={formCounts[option.value]}/>);
            })}
        </FilterSection>)}

      {/* Hearing Loss Level Filter */}
      {Object.values(lossCounts).some((count) => count > 0) && (<FilterSection title={locale === 'ru' ? 'Степень снижения слуха' : 'Eshitish darajalari'}>
          {hearingLossOptions
                ...
                return (<FilterCheckbox key={option.value} id={`loss-${option.value}`} label={locale === 'ru' ? option.label_ru : option.label_uz} checked={isSelected} url={url} count={lossCounts[option.value]}/>);
            })}
        </FilterSection>)}

      {/* Power Level Filter */}
      {Object.values(powerCounts).some((count) => count > 0) && (<FilterSection title={locale === 'ru' ? 'Мощность' : 'Quvvat'}>
          {powerLevelOptions
                ...
                return (<FilterCheckbox key={option.value} id={`power-${option.value}`} label={locale === 'ru' ? option.label_ru : option.label_uz} checked={isSelected} url={url} count={powerCounts[option.value]}/>);
            })}
        </FilterSection>)}

      {/* Clear Filters */}
      {hasActiveFilters && (<link_1.default href={`/catalog/${categorySlug}`} className="inline-block w-full rounded-full border border-brand-primary/30 bg-white px-4 py-2 text-center text-sm font-semibold text-brand-primary transition hover:bg-brand-primary/10">
          {locale === 'ru' ? 'Сбросить фильтры' : 'Filtrlarni tozalash'}
        </link_1.default>)}
    </div>);
}
//# sourceMappingURL=catalog-filters.js.map