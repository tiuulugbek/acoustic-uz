...
function FAQAccordion({ faqs, locale }) {
    const [openFaqId, setOpenFaqId] = (0, react_1.useState)(null);
    ...
    const renderFAQItem = (item) => {
        ...
        return (<div key={item.id} className="group rounded-lg border border-border/60 bg-muted/30 p-4 transition hover:border-brand-primary/50 hover:shadow-sm">
        <button id={buttonId} type="button" aria-expanded={isOpen} aria-controls={answerId} onClick={() => handleToggle(item.id)} onKeyDown={(e) => {
          ...
          <lucide_react_1.ChevronDown aria-hidden="true" className={`h-4 w-4 flex-shrink-0 text-muted-foreground transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}/>
        </button>
        {isOpen && (<div id={answerId} role="region" aria-labelledby={buttonId} className="mt-3 text-sm text-muted-foreground leading-relaxed" suppressHydrationWarning>
            {item.answer}
          </div>)}
      </div>);
    };
    return (<section className="bg-gray-50 py-16">
      <div className="mx-auto max-w-6xl space-y-8 px-4 md:px-6">
        ...
        {faqItems.length > 0 ? (<div className="grid gap-4 md:grid-cols-2">
