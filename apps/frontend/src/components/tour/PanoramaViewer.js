...
exports.default = PanoramaViewer;
const react_1 = require("react");
const image_utils_1 = require("@/lib/image-utils");
function PanoramaViewer({ config, locale = 'uz', className = '', onThumbnailsChange, showThumbnailsExternal }) {
    const viewerRef = (0, react_1.useRef)(null);
    const viewerInstanceRef = (0, react_1.useRef)(null);
    const overlayRef = (0, react_1.useRef)(null);
    const arrowsOverlayRef = (0, react_1.useRef)(null);
    const [isLoading, setIsLoading] = (0, react_1.useState)(true);
    const [error, setError] = (0, react_1.useState)(null);
    const [currentScene, setCurrentScene] = (0, react_1.useState)(config.default.firstScene);
    const [loadProgress, setLoadProgress] = (0, react_1.useState)(0);
    const [hotspotModal, setHotspotModal] = (0, react_1.useState)(null);
    const [currentHotspots, setCurrentHotspots] = (0, react_1.useState)([]);
    const [hoveredHotspotIndex, setHoveredHotspotIndex] = (0, react_1.useState)(null);
    const [previewReady, setPreviewReady] = (0, react_1.useState)(false);
    const [showThumbnails, setShowThumbnails] = (0, react_1.useState)(false);
    ...
    const handleSetShowThumbnails = (value) => {
        if (showThumbnailsExternal === undefined) {
            setShowThumbnails(value);
        }
        onThumbnailsChange?.(value);
    };
    const hoverTimeoutRef = (0, react_1.useRef)(new Map());
    const originalPositionRef = (0, react_1.useRef)(new Map());
    const initializationAttemptedRef = (0, react_1.useRef)(false);
    const isLoadingRef = (0, react_1.useRef)(false);
    const currentSceneRef = (0, react_1.useRef)(config.default.firstScene);
    const panoloadTimeoutRef = (0, react_1.useRef)(null);
    const panoloadFiredRef = (0, react_1.useRef)(false);
    const retryCountRef = (0, react_1.useRef)(0);
    const arrowUpdateRafRef = (0, react_1.useRef)(null);
    // Helper function to normalize panorama URLs (using shared utility)
    const normalizePanoramaUrl = (0, react_1.useCallback)((url) => {
        return (0, image_utils_1.normalizePanoramaUrl)(url || '');
    }, []);
    ...
    const prepareHotspots = (0, react_1.useCallback)((hotspots, scenes, setModal) => {
        ...
        return hotspots.map((hotspot) => {
            ...
            if (hotspot.type === 'scene' && hotspot.sceneId) {
                ...
                prepared.clickHandlerFunc = (hotspotDiv, args) => {
                ...
                prepared.createTooltipFunc = (hotspotDiv, args) => {
    ...
    const clearPanoloadTimeout = (0, react_1.useCallback)(() => {
    ...
    (0, react_1.useEffect)(() => {
        ...
        const initializeViewer = () => {
            ...
            try {
                ...
                viewerInstanceRef.current.on('panoloaderror', (err) => {
                ...
                viewerInstanceRef.current.on('scenechange', (sceneId) => {
                    ...
                    const newScene = config.scenes[sceneId];
                ...
                viewerInstanceRef.current.on('hotspotclick', (hotspot) => {
                    ...
                    try {
                        ...
                        if (scene && scene.hotSpots) {
                            ...
                            const clickedHotspot = scene.hotSpots.find((h) => {
                ...
                const createStaticArrowsFromPannellumHotspots = (sceneId, viewerConfig, pannellumHotspots) => {
                    ...
                    const originalScene = config.scenes[sceneId];
                    ...
                    pannellumHotspots.forEach((pnlmHotspot, idx) => {
                        if (!originalScene.hotSpots || idx >= originalScene.hotSpots.length)
                            return;
                ...
                const createArrowElement = (hotspot, index, sceneId, xPercent, yPercent) => {
          ...
          `;
                    ...
                    arrowEl._transformCheckInterval = transformCheckInterval;
                    arrowEl._transformObserver = observer;
                ...
                const createStaticArrows = (sceneId, viewerConfig) => {
                    ...
                    originalScene.hotSpots.forEach((hotspot, index) => {
                        ...
                        const calculateScreenPosition = (pitch, yaw) => {
    ...
    if (error) {
        return (<div className={`flex h-full min-h-[400px] items-center justify-center bg-gray-100 ${className}`}>
        <div className="text-center">
          <p className="text-red-600">{error}</p>
        </div>
      </div>);
    }
    return (<div className={`relative w-full ${className}`} style={{ aspectRatio: '16 / 9', minHeight: '400px' }}>
      {/* Loading indicator */}
      {isLoading && (<div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-gray-100">
          <div className="mb-4">
          ...
          {loadProgress > 0 && (<div className="mt-4 w-64">
              <div className="h-2 w-full overflow-hidden rounded-full bg-gray-200">
                <div className="h-full bg-brand-primary transition-all duration-300" style={{ width: `${loadProgress}%` }}/>
              </div>
            </div>)}
        </div>)}
      ...
      <div className="relative h-full w-full">
        <div ref={viewerRef} id="pannellum-viewer" className="h-full w-full" style={{
        ...
        {hoveredHotspotIndex !== null && currentHotspots[hoveredHotspotIndex] && (<div className="absolute inset-0 z-10 pointer-events-none" style={{
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
            }}>
            {/* Preview overlay - kichik va ixcham */}
            <div className="absolute rounded-lg overflow-hidden shadow-xl" style={{
                width: '280px',
                height: '180px',
              ...
              <div style={{
                ...
                {previewReady && hoveredHotspotIndex !== null && viewerInstanceRef.current && (<div style={{
                    width: '100%',
                    height: '100%',
                    position: 'relative',
                    overflow: 'hidden',
                    ...
                    if (hotspot.type === 'scene' && hotspot.sceneId) {
                        const targetScene = config.scenes[hotspot.sceneId];
      ...
      {!isLoading && (<div className="absolute bottom-2 left-1/2 z-30 flex -translate-x-1/2 items-center gap-1">
          {/* Zoom In */}
          <button onClick={() => {
                if (viewerInstanceRef.current) {
                    const currentHfov = viewerInstanceRef.current.getHfov() || 100;
            ...
            }} className="flex h-8 w-8 items-center justify-center rounded-full bg-white/90 text-gray-700 shadow-lg backdrop-blur-sm transition-all hover:bg-white hover:shadow-xl active:scale-95" aria-label="Zoom in">
          ...
          <button onClick={() => {
            ...
            }} className="flex h-8 w-8 items-center justify-center rounded-full bg-white/90 text-gray-700 shadow-lg backdrop-blur-sm transition-all hover:bg-white hover:shadow-xl active:scale-95" aria-label="Zoom out">
            <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button onClick={(e) => {
            ...
            }} className="flex h-8 w-8 items-center justify-center rounded-full bg-white/90 text-gray-700 shadow-lg backdrop-blur-sm transition-all hover:bg-white hover:shadow-xl active:scale-95" aria-label={locale === 'ru' ? 'Меню по панорам' : 'Panorama menyusi'}>
            <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button onClick={async () => {
            ...
            }} className="flex h-8 w-8 items-center justify-center rounded-full bg-white/90 text-gray-700 shadow-lg backdrop-blur-sm transition-all hover:bg-white hover:shadow-xl active:scale-95" aria-label={locale === 'ru' ? 'Полноэкранный режим' : 'To\'liq ekran'}>
            <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {actualShowThumbnails && !isLoading && (<>
          ...
          <div className="fixed inset-0 z-[90] bg-black/20" onClick={() => {
          ...
          <div className="fixed left-1/2 z-[100] -translate-x-1/2 bg-black/90 px-3 py-2 rounded-lg relative max-w-[90vw] shadow-2xl" style={{
                minHeight: '80px',
                pointerEvents: 'auto',
                display: 'flex',
                visibility: 'visible',
            ...
            <button onClick={() => handleSetShowThumbnails(false)} className="absolute flex h-8 w-8 items-center justify-center rounded-full bg-white text-gray-700 shadow-xl transition-all hover:bg-gray-100 hover:shadow-2xl active:scale-95" aria-label="Close" style={{
                top: '-16px',
                right: '-16px',
                zIndex: 110,
                position: 'absolute'
            ...
            <div className="flex items-center justify-center gap-2 overflow-x-auto max-w-full">
              {(() => {
                ...
                if (scenes.length === 0) {
                    return (<div className="px-4 py-2 text-white text-sm">
                ...
                return scenes.map((scene) => {
                    ...
                    return (<button key={sceneId} onClick={() => {
                            ...
                            handleSetShowThumbnails(false);
                        }} className={`relative flex-shrink-0 overflow-hidden rounded border-2 transition-all hover:scale-105 ${isActive ? 'border-white shadow-lg' : 'border-gray-400'}`} style={{ width: '80px', height: '60px' }}>
                      {panoramaUrl ? (<img src={panoramaUrl} alt={typeof scene.title === 'string' ? scene.title : (scene.title?.[locale] || `Scene ${sceneId}`)} className="h-full w-full object-cover" loading="lazy" onError={(e) => {
                      ...
                      {isActive && (<div className="absolute right-1 top-1 flex h-3 w-3 items-center justify-center rounded-full bg-white">
      ...
      {!isLoading && currentHotspots.length > 0 && (<div className="absolute bottom-12 left-1/2 z-20 flex -translate-x-1/2 flex-wrap items-center justify-center gap-1 px-4">
          {currentHotspots.map((hotspot, index) => {
                ...
                return (<button key={index} onClick={handleHotspotClick} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className="rounded-lg bg-brand-primary px-4 py-2 text-sm font-medium text-white shadow-lg transition-all hover:bg-brand-primary/90 hover:shadow-xl active:scale-95" style={{
                        border: 'none',
                    }}>
      ...
      {/* Hotspot Modal */}
      {hotspotModal && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setHotspotModal(null)}>
          <div className="relative max-w-2xl rounded-lg bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setHotspotModal(null)} className="absolute right-4 top-4 text-gray-400 hover:text-gray-600" aria-label="Close">
            ...
            {hotspotModal.description && (<p className="text-muted-foreground whitespace-pre-line">
