...
export async function generateMetadata({ params }: PostPageProps): Promise<Metadata> {
  ...
  // Verify post belongs to patients section
  ...
  if (postCategory && postCategory.section !== 'patients') {
    return {
  ...
  const postUrl = `${baseUrl}/post/patients/${params.slug}`;
...
export default async function PostPage({ params }: PostPageProps) {
  ...
  // Get related posts from patients section
  const patientsCategories = categories.filter(c => c.section === 'patients').map(c => c.id);
  ...
  const relatedPosts = allPosts
    .filter(p => {
      ...
      return patientsCategories.includes(p.categoryId);
    })
    .slice(0, 3);

  // Build Article Structured Data
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://acoustic.uz';
  const postUrl = `${baseUrl}/post/patients/${post.slug}`;
  ...
  const articleJsonLd = {
    ...
    articleSection: categoryName || (locale === 'ru' ? 'Пациентам' : 'Bemorlar'),
  };
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        '@type': 'ListItem',
        position: 2,
        name: locale === 'ru' ? 'Пациентам' : 'Bemorlar',
        item: `${baseUrl}/patients`,
      },
      {
        '@type': 'ListItem',
        position: 3,
    ...
    <main className="min-h-screen bg-background">
      ...
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { 
            label: locale === 'ru' ? 'Пациентам' : 'Bemorlar', 
            href: '/patients' 
          },
          { label: title },
        ]}
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {relatedPosts.map((relatedPost) => {
                ...
                return (
                  <Link
                    key={relatedPost.id}
                    href={`/post/patients/${relatedPost.slug}`}
