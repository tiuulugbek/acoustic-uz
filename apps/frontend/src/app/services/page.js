...
const branches_sidebar_1 = __importDefault(require("@/components/branches-sidebar"));
...
async function ServicesPage() {
    ...
    const categoriesData = await (0, api_server_1.getServiceCategories)(locale);
    // Fetch branches for sidebar map
    const branches = await (0, api_server_1.getBranches)(locale);
    ...
    const topLevelCategories = (categoriesData || []).filter((cat) => !cat.parentId && cat.status === 'published');
    ...
    const categories = topLevelCategories.map((category) => {
        ...
        const image = category.image?.url ? (0, image_utils_1.normalizeImageUrl)(category.image.url) : '';
      ...
      <section className="bg-white py-8">
        <div className="mx-auto max-w-6xl px-4 md:px-6">
          <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
            <div>
              {/* Hearing Test Card */}
              <link_1.default href="/services/hearing-test" className="group flex gap-4 rounded-lg bg-gradient-to-br from-brand-primary/10 to-brand-primary/5 border-2 border-brand-primary/20 p-6 mb-6 transition hover:border-brand-primary/40 hover:shadow-lg">
            ...
            : 'Bepul online eshitish testi. Eshitishingizni 3 daqiqada tekshiring. Natijalar tibbiy tashxis emas.'}
              ...
              {categories.length > 0 ? (<div className="grid gap-6 sm:grid-cols-2">
              {/* Optimize images with priority for above-the-fold items */}
              {categories.map((category, index) => (<link_1.default key={category.id} href={category.slug && category.slug !== '#' ? `/services/${category.slug}` : '#'} className="group flex gap-4 rounded-lg bg-white p-4 transition hover:bg-gray-50">
                  {/* Rasm - chapda */}
                  <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-lg bg-muted/20">
                    {category.image ? (<image_1.default src={category.image} alt={category.name} fill sizes="96px" className="object-cover transition-transform duration-300 group-hover:scale-105" loading={index < 4 ? 'eager' : 'lazy'} priority={index < 4} suppressHydrationWarning/>) : (<div className="flex h-full w-full items-center justify-center bg-brand-primary">
                  ...
                  <div className="flex flex-col flex-1 min-w-0">
                    ...
                    {category.description && (<p className="text-sm leading-relaxed text-muted-foreground line-clamp-3" suppressHydrationWarning>
            ...
            <aside className="space-y-6">
              ...
              <branches_sidebar_1.default locale={locale} branches={branches || []}/>
            </aside>
          </div>
        </div>
      </section>
