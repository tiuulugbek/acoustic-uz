...
const contact_form_1 = __importDefault(require("@/components/contact-form"));
          ...
          <aside className="space-y-6">
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-border/50">
              ...
              <div className="space-y-6">
                {/* Phone */}
                {phonePrimary && (<div className="flex items-start gap-4">
                    <div className="p-2 rounded-lg bg-brand-primary/10 flex-shrink-0">
                      <lucide_react_1.Phone className="h-5 w-5 text-brand-primary"/>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-muted-foreground mb-1" suppressHydrationWarning>
                        {isRu ? 'Телефон' : 'Telefon'}
                      </p>
                      <a href={`tel:${phonePrimary.replace(/\s/g, '')}`} className="text-lg font-semibold text-brand-primary hover:text-brand-accent transition-colors block">
                        {phonePrimary}
                      </a>
                      {phoneSecondary && (<a href={`tel:${phoneSecondary.replace(/\s/g, '')}`} className="text-base text-foreground hover:text-brand-primary transition-colors block mt-1">
                          {phoneSecondary}
                        </a>)}
                    </div>
                  </div>)}
                ...
                {/* Email */}
                {email && (<div className="flex items-start gap-4">
                    <div className="p-2 rounded-lg bg-brand-primary/10 flex-shrink-0">
                      <lucide_react_1.Mail className="h-5 w-5 text-brand-primary"/>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-muted-foreground mb-1" suppressHydrationWarning>
                        Email
                      </p>
                      <a href={`mailto:${email}`} className="text-base text-brand-primary hover:text-brand-accent transition-colors break-all">
                        {email}
                      </a>
                    </div>
                  </div>)}
                ...
                {/* Branches Link */}
                {branches && branches.length > 0 && (<div className="flex items-start gap-4">
                    <div className="p-2 rounded-lg bg-brand-primary/10 flex-shrink-0">
                      <lucide_react_1.MapPin className="h-5 w-5 text-brand-primary"/>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-muted-foreground mb-1" suppressHydrationWarning>
                        {isRu ? 'Адреса филиалов' : 'Filial manzillari'}
                      </p>
                      <link_1.default href="/branches" className="text-base text-brand-primary hover:text-brand-accent transition-colors inline-flex items-center gap-1" suppressHydrationWarning>
            ...
            {/* Branches Card */}
            {branches && branches.length > 0 && (<div className="bg-white rounded-xl p-6 shadow-sm border border-border/50">
                ...
                return (<link_1.default key={branch.id} href={`/branches/${branch.slug || branch.id}`} className="block p-3 rounded-lg border border-border/60 bg-muted/30 hover:border-brand-primary/50 hover:bg-white transition-colors group">
                        <p className="font-medium text-foreground group-hover:text-brand-primary transition-colors" suppressHydrationWarning>
                          {branchName}
                        </p>
                        {branch.phone && (<p className="text-sm text-muted-foreground mt-1">
                            {branch.phone}
                          </p>)}
                      </link_1.default>);
            })}
                  {branches.length > 3 && (<link_1.default href="/branches" className="block text-center text-sm text-brand-primary hover:text-brand-accent transition-colors font-medium pt-2" suppressHydrationWarning>
