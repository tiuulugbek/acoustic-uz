...
exports.default = CatalogCategoryPage;
const image_1 = __importDefault(require("next/image"));
const link_1 = __importDefault(require("next/link"));
const api_server_1 = require("@/lib/api-server");
const catalog_filters_1 = __importDefault(require("@/components/catalog-filters"));
const catalog_sort_1 = __importDefault(require("@/components/catalog-sort"));
const catalog_brand_chips_1 = __importDefault(require("@/components/catalog-brand-chips"));
const catalog_pagination_1 = __importDefault(require("@/components/catalog-pagination"));
...
function calculateFacetCounts(products) {
    const brandCounts = {};
...
async function generateMetadata({ params }) {
    ...
    const catalog = await (0, api_server_1.getCatalogBySlug)(params.slug, locale);
    const category = !catalog ? await (0, api_server_1.getCategoryBySlug)(params.slug, locale) : null;
    if (!catalog && !category) {
        return {
    ...
    const pageName = catalog
        ? (0, locale_1.getBilingualText)(catalog.name_uz, catalog.name_ru, locale)
...
async function CatalogCategoryPage({ params, searchParams }) {
    const locale = (0, locale_server_1.detectLocale)();
    // Check if slug is a brand slug
    const brands = await (0, api_server_1.getBrands)(locale);
    ...
    if (!brand) {
        ...
        catalog = await (0, api_server_1.getCatalogBySlug)(params.slug, locale);
        if (!catalog) {
            category = await (0, api_server_1.getCategoryBySlug)(params.slug, locale);
        }
    }
    ...
    if (!catalog && !category) {
        ...
        const allProductsResponse = await (0, api_server_1.getProducts)({
            status: 'published',
            limit: 1000, // Temporary: fetch many to support client-side filtering
            offset: 0,
            sort: effectiveSearchParams.sort || 'newest',
        ...
        const sortBy = effectiveSearchParams.sort || 'newest';
        ...
        const placeholderImage = `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="400" height="400"><rect width="100%" height="100%" fill="#F07E22"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#fff" font-family="Arial" font-size="28">Acoustic</text></svg>`)}`;
          ...
          <div className="mx-auto max-w-6xl px-4 md:px-6">
            <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
              ...
              <aside className="hidden lg:block">
                <catalog_filters_1.default categorySlug={params.slug} locale={locale} brands={brand ? [] : availableBrands} selectedBrands={selectedBrands} selectedBrandName={brand ? brandName : undefined} selectedAudience={selectedAudience} selectedForms={selectedForms} selectedPower={selectedPower} selectedLoss={selectedLoss} audienceCounts={audienceCounts} formCounts={formCounts} powerCounts={powerCounts} lossCounts={lossCounts}/>
              </aside>

              {/* Product Grid */}
              <div className="space-y-6">
                {/* Brand Chips - only show if not a brand page */}
                {!brand && availableBrands.length > 0 && (<catalog_brand_chips_1.default categorySlug={params.slug} locale={locale} brands={availableBrands} selectedBrands={effectiveSearchParams.brand?.split(',').filter(Boolean) ?? []}/>)}
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <catalog_sort_1.default categorySlug={params.slug} locale={locale} currentSort={sortBy}/>
                </div>
                ...
                {paginatedProducts.length > 0 ? (<>
                    ...
                    const mainImage = rawImage ? (0, image_utils_1.normalizeImageUrl)(rawImage) : placeholderImage;
                    const priceFormatted = formatPrice(product.price);
                    const availability = product.availabilityStatus ? availabilityMap[product.availabilityStatus] : undefined;
                    const productName = (0, locale_1.getBilingualText)(product.name_uz, product.name_ru, locale);
                    return (<link_1.default key={product.id} href={`/products/${product.slug}`} className="group flex flex-col gap-3 sm:gap-4 rounded-2xl border border-border/60 bg-white p-3 sm:p-5 shadow-sm transition hover:-translate-y-1 hover:border-brand-primary/50 hover:shadow-lg">
                            <div className="relative aspect-square w-full overflow-hidden rounded-xl bg-brand-primary/5">
                              <image_1.default src={mainImage} alt={productName} fill sizes="(max-width: 640px) 50vw, (max-width: 1024px) 50vw, 33vw" className="object-contain p-2 sm:p-4 transition-transform duration-300 group-hover:scale-105"/>
                            </div>
                            <div className="space-y-1.5 sm:space-y-2">
                              ...
                              {availability && (<span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-semibold ${availability.color}`}>
                    ...
                    {/* Pagination */}
                    <catalog_pagination_1.default categorySlug={params.slug} locale={locale} currentPage={currentPage} totalPages={totalPages} totalItems={totalItems}/>
    ...
    if (catalog) {
        ...
        const productsResponse = await (0, api_server_1.getProducts)({
            status: 'published',
        ...
        const allCatalogProductsResponse = await (0, api_server_1.getProducts)({
            status: 'published',
            catalogId: catalog.id,
        ...
        const placeholderImage = `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="400" height="400"><rect width="100%" height="100%" fill="#F07E22"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#fff" font-family="Arial" font-size="28">Acoustic</text></svg>`)}`;
        const catalogName = (0, locale_1.getBilingualText)(catalog.name_uz, catalog.name_ru, locale);
        return (<main className="min-h-screen bg-background">
        {/* Breadcrumbs */}
        <section className="bg-muted/40">
          <div className="mx-auto max-w-6xl px-4 py-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground md:px-6">
        ...
        <section className="bg-white py-8">
          <div className="mx-auto max-w-6xl px-4 md:px-6">
            <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
              {/* Filter Sidebar */}
              <aside className="hidden lg:block">
                <catalog_filters_1.default categorySlug={params.slug} locale={locale} brands={availableBrands} selectedBrands={searchParams.brand?.split(',').filter(Boolean) ?? []} selectedAudience={searchParams.audience?.split(',').filter(Boolean) ?? []} selectedForms={searchParams.form?.split(',').filter(Boolean) ?? []} selectedPower={searchParams.power?.split(',').filter(Boolean) ?? []} selectedLoss={searchParams.loss?.split(',').filter(Boolean) ?? []} audienceCounts={audienceCounts} formCounts={formCounts} powerCounts={powerCounts} lossCounts={lossCounts}/>
              </aside>

              {/* Product Grid */}
              <div className="space-y-6">
                {/* Brand Chips - only show if not a brand page */}
                {!brand && availableBrands.length > 0 && (<catalog_brand_chips_1.default categorySlug={params.slug} locale={locale} brands={availableBrands} selectedBrands={searchParams.brand?.split(',').filter(Boolean) ?? []}/>)}
                ...
                {paginatedProducts.length > 0 ? (<>
                    ...
                    return (<link_1.default key={product.id} href={`/products/${product.slug}`} className="group flex flex-col gap-3 sm:gap-4 rounded-2xl border border-border/60 bg-white p-3 sm:p-5 shadow-sm transition hover:-translate-y-1 hover:border-brand-primary/50 hover:shadow-lg">
                            <div className="relative aspect-square w-full overflow-hidden rounded-xl bg-brand-primary/5">
                              <image_1.default src={mainImage} alt={productName} fill className="object-contain p-2 sm:p-4 transition-transform group-hover:scale-105" sizes="(max-width: 640px) 50vw, (max-width: 1024px) 50vw, 33vw"/>
                            </div>
                            <div className="flex flex-col gap-1.5 sm:gap-2">
                              {product.brand && (<p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                                  {product.brand.name}
                                </p>)}
                              <h3 className="line-clamp-2 text-sm sm:text-base font-semibold leading-tight" suppressHydrationWarning>{productName}</h3>
                              {priceFormatted && (<p className="text-base sm:text-lg font-bold text-brand-primary" suppressHydrationWarning>{priceFormatted}</p>)}
                              {availability && (<span className={`inline-block w-fit rounded-full px-2 py-1 text-xs font-medium ${availability.color}`} suppressHydrationWarning>
                                  {locale === 'ru' ? availability.ru : availability.uz}
                                </span>)}
                            </div>
                          </link_1.default>);
                    ...
                    <catalog_pagination_1.default currentPage={currentPage} totalPages={totalPages} href={`/catalog/${params.slug}`} searchParams={searchParams}/>
    ...
    if (usesPropertyBasedMatching) {
        ...
        productsResponse = await (0, api_server_1.getProducts)({
            status: 'published',
        ...
        const allCategoryProductsResponse = await (0, api_server_1.getProducts)({
            status: 'published',
            categoryId: category.id,
            limit: 1000, // Fetch all for counts
            offset: 0,
    ...
    const placeholderImage = `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="400" height="400"><rect width="100%" height="100%" fill="#F07E22"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#fff" font-family="Arial" font-size="28">Acoustic</text></svg>`)}`;
    const categoryName = (0, locale_1.getBilingualText)(category.name_uz, category.name_ru, locale);
    return (<main className="min-h-screen bg-background">
      {/* Breadcrumbs */}
      <section className="bg-muted/40">
        <div className="mx-auto max-w-6xl px-4 py-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground md:px-6">
      ...
      <section className="bg-white py-8">
        <div className="mx-auto max-w-6xl px-4 md:px-6">
          <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
            ...
            <div className="space-y-6">
              {/* Brand Chips */}
              {availableBrands.length > 0 && (<catalog_brand_chips_1.default categorySlug={params.slug} locale={locale} brands={availableBrands} selectedBrands={searchParams.brand?.split(',').filter(Boolean) ?? []}/>)}
              ...
              {paginatedProducts.length > 0 ? (<>
                ...
                return (<link_1.default key={product.id} href={`/products/${product.slug}`} className="group flex flex-col gap-3 sm:gap-4 rounded-2xl border border-border/60 bg-white p-3 sm:p-5 shadow-sm transition hover:-translate-y-1 hover:border-brand-primary/50 hover:shadow-lg">
                          ...
                          <div className="space-y-1.5 sm:space-y-2">
                            ...
                            {product.brand && <p className="text-xs text-muted-foreground">{product.brand.name}</p>}
                            {priceFormatted && <p className="text-base sm:text-xl font-semibold text-brand-primary">{priceFormatted}</p>}
                            {availability && (<span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-semibold ${availability.color}`} suppressHydrationWarning>
                                {locale === 'ru' ? availability.ru : availability.uz}
                              </span>)}
                          </div>
                          <span className="mt-auto inline-flex items-center gap-2 text-xs sm:text-sm font-semibold text-brand-primary group-hover:text-brand-accent" suppressHydrationWarning>
                  ...
                  {/* Pagination */}
                  <catalog_pagination_1.default categorySlug={params.slug} locale={locale} currentPage={productsResponse?.page || 1} totalPages={totalPages} totalItems={totalItems}/>
                </>) : (<div className="rounded-2xl border border-border/60 bg-muted/20 p-12 text-center">
                ...
                : "Filtrlarni o'zgartirib ko'ring yoki katalogga qayting."}
                  ...
                  <link_1.default href={`/catalog/${params.slug}`} className="mt-4 inline-block rounded-full border border-brand-primary/30 bg-white px-6 py-2 text-sm font-semibold text-brand-primary transition hover:bg-brand-primary/10">
