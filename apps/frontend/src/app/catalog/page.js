...
const mobile_filter_drawer_1 = __importDefault(require("@/components/mobile-filter-drawer"));
const image_utils_1 = require("@/lib/image-utils");
// ISR: Revalidate every 30 minutes
exports.revalidate = 1800;
...
async function CatalogPage({ searchParams, }) {
    ...
    if (searchParams.category) {
        (0, navigation_1.redirect)(`/catalog/${searchParams.category}`);
    }
    const locale = (0, locale_server_1.detectLocale)();
    // If productType is provided, show products instead of catalogs
    if (searchParams.productType) {
        const { getProducts, getProductCategories } = await Promise.resolve().then(() => __importStar(require('@/lib/api-server')));
        ...
        if (searchParams.filter === 'children') {
            filteredProducts = filteredProducts.filter((p) => p.audience?.includes('children'));
        }
        // Filter by "wireless" if filter=wireless (legacy support)
        if (searchParams.filter === 'wireless') {
            filteredProducts = filteredProducts.filter((p) => p.smartphoneCompatibility?.length > 0 ||
        ...
        <section className="bg-muted/40">
          <div className="mx-auto max-w-6xl px-4 py-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground md:px-6">
            <link_1.default href="/" className="hover:text-brand-primary" suppressHydrationWarning>
              {locale === 'ru' ? 'Главная' : 'Bosh sahifa'}
            </link_1.default>
            <span className="mx-2">›</span>
            <link_1.default href="/catalog" className="hover:text-brand-primary" suppressHydrationWarning>
          ...
          <div className="mx-auto max-w-6xl px-4 md:px-6">
            <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
              ...
              <aside className="hidden lg:block space-y-6">
                ...
                {searchParams.productType !== 'interacoustics' && (<div className="rounded-lg border border-border/60 bg-white p-4">
                  ...
                  {searchParams.productType !== 'interacoustics' && (<div className="mb-6">
                      ...
                      <div className="space-y-2">
                        ...
                        if (searchParams.productType)
                            params.set('productType', searchParams.productType);
                        ...
                        if (searchParams.audience)
                            params.set('audience', searchParams.audience);
                        if (searchParams.formFactor)
                            params.set('formFactor', searchParams.formFactor);
                        if (searchParams.signalProcessing)
                            params.set('signalProcessing', searchParams.signalProcessing);
                        if (searchParams.powerLevel)
                            params.set('powerLevel', searchParams.powerLevel);
                        if (searchParams.hearingLossLevel)
                            params.set('hearingLossLevel', searchParams.hearingLossLevel);
                        if (searchParams.smartphoneCompatibility)
                            params.set('smartphoneCompatibility', searchParams.smartphoneCompatibility);
                        return (<link_1.default key={brand.id} href={`/catalog?${params.toString()}`} className="flex items-center gap-2 hover:opacity-80">
                              <div className={`h-4 w-4 rounded border-2 flex items-center justify-center ${isChecked
                                ? 'border-brand-primary bg-brand-primary'
                                : 'border-border/60 bg-white'}`}>
                                {isChecked && (<svg className="h-3 w-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        if (searchParams.brandId)
                            params.set('brandId', searchParams.brandId);
                        if (searchParams.audience)
                            params.set('audience', searchParams.audience);
                        if (!isChecked)
                            params.set('formFactor', option.value);
                        // Preserve other filters
                        if (searchParams.signalProcessing)
                            params.set('signalProcessing', searchParams.signalProcessing);
                        if (searchParams.powerLevel)
                        ...
                        if (searchParams.smartphoneCompatibility)
                            params.set('smartphoneCompatibility', searchParams.smartphoneCompatibility);
                        return (<link_1.default key={option.value} href={`/catalog?${params.toString()}`} className="flex items-center gap-2 hover:opacity-80">
                            <div className={`h-4 w-4 rounded border-2 flex items-center justify-center ${isChecked
                                ? 'border-brand-primary bg-brand-primary'
                                : 'border-border/60 bg-white'}`}>
                              {isChecked && (<svg className="h-3 w-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        if (searchParams.formFactor)
                            params.set('formFactor', searchParams.formFactor);
                        if (!isChecked)
                            params.set('signalProcessing', option.value);
                        // Preserve other filters
                        if (searchParams.powerLevel)
                            params.set('powerLevel', searchParams.powerLevel);
                        if (searchParams.hearingLossLevel)
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        if (searchParams.signalProcessing)
                            params.set('signalProcessing', searchParams.signalProcessing);
                        if (!isChecked)
                            params.set('powerLevel', option.value);
                        // Preserve other filters
                        if (searchParams.hearingLossLevel)
                            params.set('hearingLossLevel', searchParams.hearingLossLevel);
                        if (searchParams.smartphoneCompatibility)
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        if (searchParams.powerLevel)
                            params.set('powerLevel', searchParams.powerLevel);
                        if (!isChecked)
                            params.set('hearingLossLevel', option.value);
                        // Preserve other filters
                        if (searchParams.smartphoneCompatibility)
                            params.set('smartphoneCompatibility', searchParams.smartphoneCompatibility);
                        return (<link_1.default key={option.value} href={`/catalog?${params.toString()}`} className="flex items-center gap-2 hover:opacity-80">
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        if (searchParams.hearingLossLevel)
                            params.set('hearingLossLevel', searchParams.hearingLossLevel);
                        if (!isChecked)
                            params.set('smartphoneCompatibility', option.value);
                        return (<link_1.default key={option.value} href={`/catalog?${params.toString()}`} className="flex items-center gap-2 hover:opacity-80">
                            <div className={`h-4 w-4 rounded border-2 flex items-center justify-center ${isChecked
                                ? 'border-brand-primary bg-brand-primary'
                                : 'border-border/60 bg-white'}`}>
                ...
                <div className="space-y-3">
                  <div className="space-y-2">
                    <link_1.default href="/catalog?productType=hearing-aids" className="flex items-center gap-3 rounded-lg border border-border/60 bg-muted/30 p-3 transition hover:border-brand-primary/50 hover:bg-white">
                ...
                {/* Useful Articles */}
                {postsData && postsData.length > 0 && (<div className="space-y-3">
                    ...
                    const rawCoverImage = post.cover?.url || '';
                    const coverImage = (0, image_utils_1.normalizeImageUrl)(rawCoverImage);
                    return (<link_1.default key={post.id} href={`/posts/${post.slug}`} className="group flex items-start gap-3 rounded-lg transition hover:opacity-80">
                            {coverImage ? (<div className="relative h-16 w-16 shrink-0 overflow-hidden rounded bg-muted/30">
                                <image_1.default src={coverImage} alt={title} fill sizes="64px" className="object-cover" suppressHydrationWarning/>
                              </div>) : (<div className="relative h-16 w-16 shrink-0 overflow-hidden rounded bg-muted/30">
              ...
              <div className="space-y-6">
                ...
                {searchParams.productType === 'hearing-aids' && settingsData?.catalogHeroImage?.url && (<section className="relative w-full rounded-lg overflow-hidden">
                    <div className="relative aspect-[21/9] w-full">
                      <image_1.default src={(0, image_utils_1.normalizeImageUrl)(settingsData.catalogHeroImage.url)} alt={locale === 'ru' ? 'Каталог слуховых аппаратов' : 'Eshitish moslamalari katalogi'} fill className="object-cover" sizes="(max-width: 768px) 100vw, 1200px" priority/>
                    </div>
                  </section>)}

                {/* 2. Brand Tabs - Second */}
                {brandTabs.length > 0 && searchParams.productType !== 'interacoustics' && (<div className="flex flex-wrap gap-2">
                    <link_1.default href={`/catalog?productType=${searchParams.productType}`} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${!searchParams.brandId
                    ...
                    if (searchParams.smartphoneCompatibility)
                        params.set('smartphoneCompatibility', searchParams.smartphoneCompatibility);
                    return (<link_1.default key={brand.id} href={`/catalog?${params.toString()}`} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${searchParams.brandId === brand.id
                            ? 'bg-brand-primary text-white'
                            : 'bg-muted/30 text-foreground hover:bg-muted/50'}`}>
                          {brand.name}
                        </link_1.default>);
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  ...
                  <link_1.default href={`/catalog?productType=${searchParams.productType}&sort=price_asc${searchParams.brandId ? `&brandId=${searchParams.brandId}` : ''}`} className={`hover:text-brand-primary ${searchParams.sort === 'price_asc' ? 'text-brand-primary font-medium' : ''}`}>
                    {locale === 'ru' ? 'цене' : 'narx'}
                  </link_1.default>
                </div>
                ...
                {filteredProducts.length > 0 ? (<>
                    ...
                    if (product.galleryUrls && product.galleryUrls.length > 0) {
                        mainImage = (0, image_utils_1.normalizeImageUrl)(product.galleryUrls[0]);
                    }
                    else if (product.brand?.logo?.url) {
                        mainImage = (0, image_utils_1.normalizeImageUrl)(product.brand.logo.url);
                    }
                    ...
                    const availability = product.availabilityStatus
                        ...
                        : null;
                    return (<link_1.default key={product.id} href={`/products/${product.slug}`} className="group flex flex-col gap-2 sm:gap-3 rounded-lg border-[0.5px] border-brand-accent/40 bg-white p-2 sm:p-4 shadow-sm transition hover:border-brand-accent/60 hover:shadow-md">
                            ...
                            <div className="relative aspect-square w-full overflow-hidden rounded-lg border border-brand-accent/60 bg-white">
                              {mainImage ? (<image_1.default src={mainImage} alt={productName} fill sizes="(max-width: 640px) 50vw, (max-width: 1024px) 50vw, 33vw" className="object-contain p-2 sm:p-4 transition-transform duration-300 group-hover:scale-105" suppressHydrationWarning unoptimized/>) : (<div className="flex h-full w-full items-center justify-center bg-brand-primary">
                            ...
                            <div className="flex flex-col gap-1 sm:gap-2">
                              ...
                              {priceFormatted && (<p className="text-base sm:text-lg font-semibold text-brand-primary" suppressHydrationWarning>
                                  {priceFormatted}
                                </p>)}
                              {availability && (<p className="text-xs sm:text-sm text-emerald-600 font-medium" suppressHydrationWarning>
                              ...
                              <span className="inline-flex items-center gap-1 text-xs sm:text-sm font-medium text-brand-primary group-hover:gap-2 transition-all mt-auto" suppressHydrationWarning>
                                ...
                                <lucide_react_1.ArrowRight size={12} className="sm:w-3.5 sm:h-3.5 transition-transform group-hover:translate-x-1"/>
                              </span>
                            </div>
                          </link_1.default>);
                })}
                    ...
                    {totalPages > 1 && (<div className="flex items-center justify-center gap-2 pt-6">
                        {page > 1 && (<link_1.default href={`/catalog?productType=${searchParams.productType}&page=${page - 1}${searchParams.brandId ? `&brandId=${searchParams.brandId}` : ''}${searchParams.sort ? `&sort=${searchParams.sort}` : ''}${searchParams.audience ? `&audience=${searchParams.audience}` : ''}${searchParams.formFactor ? `&formFactor=${searchParams.formFactor}` : ''}${searchParams.signalProcessing ? `&signalProcessing=${searchParams.signalProcessing}` : ''}${searchParams.powerLevel ? `&powerLevel=${searchParams.powerLevel}` : ''}${searchParams.hearingLossLevel ? `&hearingLossLevel=${searchParams.hearingLossLevel}` : ''}${searchParams.smartphoneCompatibility ? `&smartphoneCompatibility=${searchParams.smartphoneCompatibility}` : ''}`} className="px-4 py-2 rounded-lg border border-border/60 bg-white text-sm font-medium text-foreground transition hover:border-brand-primary/50 hover:bg-muted/20">
                        ...
                        if (searchParams.productType)
                            pageParams.set('productType', searchParams.productType);
                        if (searchParams.brandId)
                            pageParams.set('brandId', searchParams.brandId);
                        if (searchParams.sort)
                            pageParams.set('sort', searchParams.sort);
                        if (searchParams.audience)
                            pageParams.set('audience', searchParams.audience);
                        if (searchParams.formFactor)
                            pageParams.set('formFactor', searchParams.formFactor);
                        if (searchParams.signalProcessing)
                            pageParams.set('signalProcessing', searchParams.signalProcessing);
                        if (searchParams.powerLevel)
                            pageParams.set('powerLevel', searchParams.powerLevel);
                        if (searchParams.hearingLossLevel)
                            pageParams.set('hearingLossLevel', searchParams.hearingLossLevel);
                        if (searchParams.smartphoneCompatibility)
                            pageParams.set('smartphoneCompatibility', searchParams.smartphoneCompatibility);
                        pageParams.set('page', pageNum.toString());
                        return (<link_1.default key={pageNum} href={`/catalog?${pageParams.toString()}`} className={`px-3 py-1.5 rounded text-sm font-medium transition ${isActive
                                ? 'bg-brand-primary text-white'
                        ...
                        {page < totalPages && (<link_1.default href={`/catalog?productType=${searchParams.productType}&page=${page + 1}${searchParams.brandId ? `&brandId=${searchParams.brandId}` : ''}${searchParams.sort ? `&sort=${searchParams.sort}` : ''}${searchParams.audience ? `&audience=${searchParams.audience}` : ''}${searchParams.formFactor ? `&formFactor=${searchParams.formFactor}` : ''}${searchParams.signalProcessing ? `&signalProcessing=${searchParams.signalProcessing}` : ''}${searchParams.powerLevel ? `&powerLevel=${searchParams.powerLevel}` : ''}${searchParams.hearingLossLevel ? `&hearingLossLevel=${searchParams.hearingLossLevel}` : ''}${searchParams.smartphoneCompatibility ? `&smartphoneCompatibility=${searchParams.smartphoneCompatibility}` : ''}`} className="px-4 py-2 rounded-lg border border-border/60 bg-white text-sm font-medium text-foreground transition hover:border-brand-primary/50 hover:bg-muted/20">
        ...
        {searchParams.productType && searchParams.productType !== 'interacoustics' && (<mobile_filter_drawer_1.default locale={locale} brandTabs={brandTabs} searchParams={searchParams}/>)}
      </main>);
    }
    ...
    const [catalogsData, postsData, brandsData, settingsData] = await Promise.all([
        (0, api_server_1.getCatalogs)(locale),
        (0, api_server_1.getPosts)(locale, true),
        (0, api_server_1.getBrands)(locale),
        (0, api_server_1.getSettings)(locale),
    ]);
    ...
    if (settingsData?.sidebarBrandIds && settingsData.sidebarBrandIds.length > 0) {
        ...
        mainBrands = brandsData?.filter((brand) => settingsData.sidebarBrandIds.includes(brand.id)) || [];
    }
    else {
        ...
        mainBrands = brandsData?.filter((brand) => {
            ...
            return (brandName.includes('oticon') ||
    ...
    return (<main className="min-h-screen bg-background">
      ...
      <section className="bg-[hsl(var(--secondary))]">
        <div className="mx-auto max-w-6xl px-4 py-2 sm:py-3 text-xs font-semibold uppercase tracking-wide text-white sm:px-6">
          <div className="flex flex-wrap items-center gap-x-2">
            <link_1.default href="/" className="hover:text-white/80 text-white/70 break-words" suppressHydrationWarning>
              {locale === 'ru' ? 'Главная' : 'Bosh sahifa'}
            </link_1.default>
        ...
        <div className="mx-auto max-w-6xl px-4 md:px-6">
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            : "Biz sizning odatlaringiz, faolligingiz va byudjetingizga mos modelni topamiz. Bo'limlardan birini tanlang, keyin esa katalog ichida mos mahsulotlarni ko'ring."}
              ...
              {/* Catalog Cards */}
              {catalogItems.length > 0 ? (<div className="grid gap-3 grid-cols-2 md:gap-6 md:grid-cols-2">
                  {catalogItems.map((item) => (<link_1.default key={item.id} href={item.link} className="group flex gap-2 md:gap-4 rounded-lg border border-gray-200 bg-white p-2.5 md:p-4 transition hover:border-brand-primary/50 hover:shadow-md">
                      {/* Rasm - chapda */}
                      <div className="relative h-16 w-16 md:h-24 md:w-24 shrink-0 overflow-hidden rounded-lg bg-brand-primary/10">
                        {item.image ? (<image_1.default src={item.image} alt={item.title} fill sizes="(max-width: 768px) 64px, 96px" className="object-cover transition-transform duration-300 group-hover:scale-105" suppressHydrationWarning/>) : (<div className="flex h-full w-full items-center justify-center bg-brand-primary">
                      ...
                      <div className="flex flex-col gap-1 md:gap-1.5 flex-1 min-w-0">
                        ...
                        {/* Tavsif */}
                        {item.description && (<p className="text-[10px] md:text-xs text-muted-foreground leading-relaxed line-clamp-2 md:line-clamp-3" suppressHydrationWarning>
                            {item.description}
                          </p>)}
                      </div>
                    </link_1.default>))}
            ...
            <div className="lg:col-span-1">
              <sidebar_1.default locale={locale} settingsData={settingsData} brandsData={brandsData} pageType="catalog"/>
            </div>
          </div>
        </div>
      </section>
      ...
      {postsData && postsData.length > 0 && (<section className="bg-gray-50 py-16">
          <div className="mx-auto max-w-6xl space-y-8 px-4 md:px-6">
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
              {postsData.slice(0, 4).map((post) => {
                ...
                return (<link_1.default key={post.id} href={link} className="group flex flex-col gap-3 p-4 rounded-lg bg-white border border-gray-200 hover:border-brand-primary/50 hover:shadow-md transition-all">
                    ...
                    {excerpt && (<p className="text-sm text-muted-foreground leading-relaxed line-clamp-3" suppressHydrationWarning>
                        {excerpt}
                      </p>)}
                    <span className="inline-flex items-center gap-1 text-xs font-medium text-brand-primary group-hover:gap-2 transition-all mt-auto" suppressHydrationWarning>
                      ...
                      <lucide_react_1.ArrowRight size={14} className="transition-transform group-hover:translate-x-1"/>
                    </span>
                  </link_1.default>);
            })}
            </div>
