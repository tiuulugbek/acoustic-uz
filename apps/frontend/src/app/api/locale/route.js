...
async function POST(request) {
    try {
        ...
        if (locale !== 'uz' && locale !== 'ru') {
            ...
            return server_1.NextResponse.json({ error: 'Invalid locale. Must be "uz" or "ru".' }, { status: 400 });
        }
        ...
        response.cookies.set(locale_1.LOCALE_COOKIE_NAME, locale, {
            path: '/',
        ...
        if (process.env.NODE_ENV === 'development') {
            console.log(`[Locale API] ✅ Setting cookie ${locale_1.LOCALE_COOKIE_NAME}=${locale} (POST)`);
        ...
        return server_1.NextResponse.json({ error: 'Invalid request' }, { status: 400 });
    }
}
async function GET(request) {
    ...
    // Log cookie setting for debugging
    if (process.env.NODE_ENV === 'development') {
        console.log(`[Locale API] ✅ Setting cookie ${locale_1.LOCALE_COOKIE_NAME}=${locale}`);
        console.log(`[Locale API] Redirecting to: ${redirectUrl.toString()}`);
