...
async function sitemap() {
    ...
    const addUrl = (url, lastModified, changeFrequency = 'weekly', priority = 0.8) => {
    ...
    try {
        // Add products
        const productsResponse = await (0, api_server_1.getProducts)({ page: 1, limit: 1000 }, 'uz');
    ...
    try {
        ...
        const posts = await (0, api_server_1.getPosts)('uz', true);
    ...
    try {
        // Add branches
        const branches = await (0, api_server_1.getBranches)('uz');
    ...
    try {
        // Add services
        const services = await (0, api_server_1.getServices)('uz');
    ...
    try {
        ...
        const serviceCategories = await (0, api_server_1.getServiceCategories)('uz');
    ...
    const uniqueEntries = sitemapEntries.filter((entry, index, self) => index === self.findIndex((e) => e.url === entry.url));
