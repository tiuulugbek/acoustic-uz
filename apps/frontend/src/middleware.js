...
function middleware(request) {
    ...
    const localeCookie = request.cookies.get(locale_1.LOCALE_COOKIE_NAME);
    ...
    if (!localeCookie || !localeCookie.value) {
        ...
        response.cookies.set(locale_1.LOCALE_COOKIE_NAME, locale_1.DEFAULT_LOCALE, {
            path: '/',
        ...
        if (process.env.NODE_ENV === 'development') {
            console.log(`[Middleware] Setting default locale cookie: ${locale_1.DEFAULT_LOCALE}`);
        }
    }
    else {
        ...
        if (value !== 'ru' && value !== 'uz') {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log(`[Middleware] Invalid cookie value "${localeCookie.value}", resetting to default: ${locale_1.DEFAULT_LOCALE}`);
            }
        }
        else {
            // Valid cookie - preserve it (ensure it's set in response to refresh expiry)
            response.cookies.set(locale_1.LOCALE_COOKIE_NAME, value, {
                path: '/',
                maxAge: 60 * 60 * 24 * 365, // 1 year
                sameSite: 'lax',
                httpOnly: false,
