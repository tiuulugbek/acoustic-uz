...
let ProductsService = (() => {
    ...
    var ProductsService = class {
        ...
        async create(data) {
            const validated = shared_1.productSchema.parse(data);
            ...
            const createData = {
                ...
                tinnitusSupport: validated.tinnitusSupport === undefined ? undefined : validated.tinnitusSupport ?? false,
        ...
        async update(id, data) {
            const validated = shared_1.productSchema.partial().parse(data);
            const updateData = {};
            ...
            if (validated.description_uz !== undefined)
                updateData.description_uz = validated.description_uz ?? null;
            if (validated.description_ru !== undefined)
                updateData.description_ru = validated.description_ru ?? null;
            if (validated.price !== undefined)
                updateData.price = validated.price ?? null;
            if (validated.stock !== undefined)
                updateData.stock = validated.stock ?? null;
            ...
            if (validated.intro_uz !== undefined)
                updateData.intro_uz = validated.intro_uz ?? null;
            if (validated.intro_ru !== undefined)
                updateData.intro_ru = validated.intro_ru ?? null;
            ...
            if (validated.tech_uz !== undefined)
                updateData.tech_uz = validated.tech_uz ?? null;
            if (validated.tech_ru !== undefined)
                updateData.tech_ru = validated.tech_ru ?? null;
            if (validated.fittingRange_uz !== undefined)
                updateData.fittingRange_uz = validated.fittingRange_uz ?? null;
            if (validated.fittingRange_ru !== undefined)
                updateData.fittingRange_ru = validated.fittingRange_ru ?? null;
            if (validated.regulatoryNote_uz !== undefined)
                updateData.regulatoryNote_uz = validated.regulatoryNote_uz ?? null;
            if (validated.regulatoryNote_ru !== undefined)
                updateData.regulatoryNote_ru = validated.regulatoryNote_ru ?? null;
        ...
        async importFromExcel(fileBuffer) {
            ...
            if (!data || data.length === 0) {
                throw new common_1.BadRequestException('Excel fayli bo\'sh');
            }
            const errors = [];
            let success = 0;
            let failed = 0;
                ...
                try {
                    ...
                    const parseBoolean = (value) => {
                        ...
                        if (typeof value === 'string') {
                            ...
                            if (lower === 'true' || lower === 'yes' || lower === 'да' || lower === 'ha')
                                return true;
                            if (lower === 'false' || lower === 'no' || lower === 'нет' || lower === 'yo\'q')
                                return false;
                        }
                        return undefined;
                    };
