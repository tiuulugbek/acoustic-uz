#!/bin/bash
# Force rebuild admin panel with cache clearing
...
echo -e "${BLUE}🔄 Force rebuilding admin panel with cache clearing...${NC}"
echo ""
...
# 2. Clean admin build directories completely
echo -e "${BLUE}🧹 Step 2: Cleaning admin build directories...${NC}"
rm -rf apps/admin/dist
rm -rf apps/admin/.vite
rm -rf apps/admin/node_modules/.vite
rm -rf apps/admin/node_modules/.cache
rm -rf apps/admin/.cache
find apps/admin -name "*.tsbuildinfo" -delete 2>/dev/null || true
echo -e "${GREEN}✅ Admin build directories cleaned${NC}"
...
echo -e "${BLUE}🏗️  Step 4: Building shared package...${NC}"
...
if [ -d "apps/admin/dist" ]; then
    ...
    if [ -f "apps/admin/src/version.json" ]; then
        ...
        echo "  Version info:"
        cat apps/admin/src/version.json | sed 's/^/    /'
    fi
else
...
# 7. Remove old admin files completely
echo -e "${BLUE}📋 Step 7: Removing old admin files...${NC}"
rm -rf "$ADMIN_DIR"/*
rm -rf "$ADMIN_DIR"/.* 2>/dev/null || true
echo -e "${GREEN}  ✅ Old files removed${NC}"

# 8. Copy admin build to Nginx root
echo -e "${BLUE}📋 Step 8: Copying admin build to Nginx root...${NC}"
mkdir -p "$ADMIN_DIR"
...
echo -e "${BLUE}📋 Step 9: Verifying copied files...${NC}"
...
# 10. Clear Nginx cache
echo -e "${BLUE}🔄 Step 10: Clearing Nginx cache...${NC}"
...
# 12. Test admin panel
echo -e "${BLUE}📋 Step 12: Testing admin panel...${NC}"
...
echo "  4. Check Nginx cache headers"
echo "  5. Check Nginx logs: tail -f /var/log/nginx/admin.acoustic.uz.error.log"
