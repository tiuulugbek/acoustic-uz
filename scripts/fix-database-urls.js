 ...
 */
function fixUrlsInContent(content) {
    if (!content)
        return content;
    // Fix URLs in img src attributes
    let fixed = content.replace(/<img([^>]*)\ssrc=["']([^"']+)["']([^>]*)>/gi, (match, before, src, after) => {
        const fixedSrc = normalizeUrl(src);
        return `<img${before} src="${fixedSrc}"${after}>`;
    });
    // Fix URLs in a href attributes (for links to images)
    fixed = fixed.replace(/<a([^>]*)\shref=["']([^"']+)["']([^>]*)>/gi, (match, before, href, after) => {
...
async function main() {
    ...
    const products = allProducts.filter(product => product.galleryUrls.some(url => url.includes('localhost') ||
    ...
    for (const product of productsWithContent) {
        ...
        const hasChanges = fixedDescriptionUz !== product.description_uz ||
...
//# sourceMappingURL=fix-database-urls.js.map